after entering the amount and the positons of stains and fruits please enter v for value iteration algorithm or p for policy iteration (get_policy method responsible for that).
constants for graphs:
  a. seconds_per_period
  b. iterations_per_period
  c. plot_graph
  if you want to find the policy without ploting the graphs (ploting the graphs affects running time performance) please change the value   of 'plot_graph' to faulse
 ...
 c. policy_iteratin, set_value_function: policy iteration simulates policy iteration algorithm, we use set_value_function in order to find     the matching value function the the given policy according to gamma and epsilon
 
 ploting graphs:
 a. collect_and_plot_graph_data: This function runs by a seprated thread and writes every constant amount of time the average
